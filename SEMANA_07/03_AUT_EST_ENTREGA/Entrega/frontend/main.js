        ...
        $.ajax({
            ...
            success: data => {
                ...
                data.forEach(element => {
                    tx += '<div class="user">';
                        tx += '<div class="title">' + `${element.Personalidade} - ${element.Score}/5 ` + '</div>';
                        tx += '<div class="actions">';
                            tx += '<div class="action" onclick="user.update(' + `${element.Cód}, '${element.Personalidade}', ${element.Score}` + ')">Editar</div>';
                            tx += '<div class="action" onclick="user.delete(' + element.Cód + ')">Excluir</div>';
                        tx += '</div>';
                    tx += '</div>';
                });
                $('#persona').html(tx);
